 ...
 */
export const useTimetables = (pointA: Coords, pointB: Coords): Timetables => {
  ...
  const date: string = `${now.getFullYear()}-${now.getMonth() +
    1}-${now.getDate()}`;
  const time: string = `${now.getHours() * 60 * 60 +
    now.getMinutes() * 60 +
    now.getSeconds()}`;
  ...
  const changeDestination = (newCoords: Coords, newName: string) => {
